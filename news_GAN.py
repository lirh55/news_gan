...
for line in train_lines:
    ...
    train_title.append(line.split('\t')[0])
...
for line in dev_lines:
    test_title.append(line.split('\t')[0])
    test_label.append(line.split('\t')[1].replace('\n', ''))

title_list = train_title + test_title
...
sequence_length = len(train_title)
num_classes = 14
...
train_data = load_dataset('dataset/train.txt', 32)
dev_data = load_dataset('dataset/dev.txt', 32)
...
def train(discriminator, generator, train_iter, dev_iter):
    ...
    for epoch in range(3):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            # 假数据的标签
            fake_labels = torch.randint(0, 14, (128,)).to(device)
        ...
        # if flag:
           #  break
    writer.close()
    model_test(discriminator, generator, dev_iter)
...
# train(discriminator, generator, train_iter, dev_iter)
model_test(discriminator, generator, dev_iter)